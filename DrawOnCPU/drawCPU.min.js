...
function B(a,b){this.width=a;this.height=b;this.left=0;this.right=0+a;this.bottom=0;this.top=0+b}var C=new Stats;C.showPanel(0);document.body.appendChild(C.dom);c.width=512;c.height=512;
for(var D=Math.min(window.innerWidth,window.innerHeight),E=(Math.max(window.innerWidth,window.innerHeight)-D)/2/D,F=new function(){this.g=.03;this.alpha=4/(Math.PI*Math.pow(.03,8))},G=new n(0,-9.8),A=new B(1,1),y=new z,H=6*y.g*c.height,I=6*y.g,L=I*I,M=[],u=new B(.5,.8),N=Math.round(u.width/.02),O=Math.round(u.height/.02),v=0;v<N;v++)for(var w=0;w<O;w++)M.push(new t);var P=1,Q=0,R=0,S=!1;
m.innerHTML="particleSize : 0.02<br>particleNum : "+M.length+"<br>canvasSize : 512<br>timeStepLength : 0.0025<br>iterationNum : 4<br>restDensity : 1000<br>viscosity : 4<br>massParticle : 0.4<br>gravity : Vec2(x : "+G.x+", y : "+G.y+")";c.addEventListener("mousedown",function(a){a.preventDefault();T(a)});c.addEventListener("mousemove",function(a){a.preventDefault();Q=a.pageX/D-E;R=1-a.pageY/D});c.addEventListener("mouseup",function(a){a.preventDefault();S=!1});
...
function U(){(function(a){for(var b={},d=0,f=a.length;d<f;b={i:b.i},d++)b.i=0,x(a[d],function(h){return function(g,e){var l=p(e);h.i+=.4*(l<F.g?F.alpha*Math.pow(F.g*F.g-l*l,3):0)}}(b)),a[d].i=b.i,a[d].pressure=Math.max(100*(a[d].i-1E3),0)})(M)}
function V(){for(var a={j:0},b=M.length;a.j<b;a={j:a.j,h:a.h},a.j++)a.h=new n,x(M[a.j],function(d){return function(f,h){if(M[d.j]!==f){var g=p(h);var e=p(h);e<F.g?(e=-6*F.alpha*Math.pow(F.g*F.g-e*e,2),e=new n(e*h.x,e*h.y)):e=new n;var l=-.4*(f.pressure/(f.i*f.i)+M[d.j].pressure/(M[d.j].i*M[d.j].i));d.h.x+=e.x*l;d.h.y+=e.y*l;l=g*g+3E-4*.03;g=M[d.j].s.sub(f.s);e=3.2/(f.i*M[d.j].i)*(h.x*e.x+h.y*e.y)/l;d.h.x+=e*g.x;d.h.y+=e*g.y}}}(a)),a.h.x+=G.x,a.h.y+=G.y,M[a.j].force.x=a.h.x,M[a.j].force.y=a.h.y}
